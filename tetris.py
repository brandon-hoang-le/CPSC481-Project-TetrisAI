...
class MainBoard:
    ...
    def restart(self):
        self.blockMat = [['empty'] * self.colNum for i in
                         range(self.rowNum)]
    ...
    def erase_BLOCK(
        ...
        pygame.draw.rect(gameDisplay, BLACK, [xRef + col
                         * self.blockSize, yRef + row * self.blockSize,
                         self.blockSize, self.blockSize], 0)
    ...
    def draw_BLOCK(
        ...
        pygame.draw.rect(gameDisplay, BLACK, [xRef + col
                         * self.blockSize + self.blockSize
                         - self.blockLineWidth, yRef + row
                         * self.blockSize, self.blockLineWidth,
                         self.blockSize], 0)
        pygame.draw.rect(gameDisplay, BLACK, [xRef + col
                         * self.blockSize, yRef + row * self.blockSize,
                         self.blockLineWidth, self.blockSize], 0)
        pygame.draw.rect(gameDisplay, BLACK, [xRef + col
                         * self.blockSize, yRef + row * self.blockSize
                         + self.blockSize - self.blockLineWidth,
                         self.blockSize, self.blockLineWidth], 0)

        pygame.draw.rect(gameDisplay, color, [xRef + col
                         * self.blockSize + self.blockLineWidth, yRef
                         + row * self.blockSize + self.blockLineWidth,
    ...
    def draw_GAMEBOARD_BORDER(self):
        pygame.draw.rect(gameDisplay, BORDER_COLOR, [self.xPos
                         ...
                         self.yPos - self.boardLineWidth
                         - self.blockLineWidth, self.blockSize
                         * self.colNum + 2 * self.boardLineWidth + 2
                         * self.blockLineWidth, self.boardLineWidth], 0)
        pygame.draw.rect(gameDisplay, BORDER_COLOR, [self.xPos
                         + self.blockSize * self.colNum
                         + self.blockLineWidth, self.yPos
                         - self.boardLineWidth - self.blockLineWidth,
                         self.boardLineWidth, self.blockSize
                         * self.rowNum + 2 * self.boardLineWidth + 2
                         * self.blockLineWidth], 0)
        pygame.draw.rect(gameDisplay, BORDER_COLOR, [self.xPos
                         - self.boardLineWidth - self.blockLineWidth,
                         self.yPos - self.boardLineWidth
                         - self.blockLineWidth, self.boardLineWidth,
                         self.blockSize * self.rowNum + 2
                         * self.boardLineWidth + 2
                         * self.blockLineWidth], 0)
        pygame.draw.rect(gameDisplay, BORDER_COLOR, [self.xPos
                         - self.boardLineWidth - self.blockLineWidth,
                         self.yPos + self.blockSize * self.rowNum
                         + self.blockLineWidth, self.blockSize
                         * self.colNum + 2 * self.boardLineWidth + 2
                         * self.blockLineWidth, self.boardLineWidth], 0)
            ...
            for row in range(0, self.rowNum):
                for col in range(0, self.colNum):
                    if self.blockMat[row][col] == 'empty':
                        ...
                        self.draw_BLOCK(self.xPos, self.yPos, row, col,
                                        blockColors[self.blockMat[row][col]])

            if self.piece.status == 'moving':
                for i in range(0, 4):
                    self.draw_BLOCK(self.xPos, self.yPos,
                                    self.piece.blocks[i].currentPos.row,
                                    self.piece.blocks[i].currentPos.col,
                                    blockColors[self.piece.type])

            if self.gamePause == True:
                pygame.draw.rect(gameDisplay, DARK_GRAY, [self.xPos + 1
                                 * self.blockSize, self.yPos + 8
                                 * self.blockSize, 8 * self.blockSize,
                                 4 * self.blockSize], 0)
                pauseText = fontPAUSE.render('PAUSE', False, BLACK)
                gameDisplay.blit(pauseText, (self.xPos + +1.65
                                 * self.blockSize, self.yPos + 8
                                 * self.blockSize))

            if self.gameStatus == 'gameOver':
                pygame.draw.rect(gameDisplay, LIGHT_GRAY, [self.xPos
                                 + 1 * self.blockSize, self.yPos + 8
                                 * self.blockSize, 8 * self.blockSize,
                                 8 * self.blockSize], 0)
                gameOverText0 = fontGAMEOVER.render('GAME', False,
                                                    BLACK)
                gameDisplay.blit(gameOverText0, (self.xPos + +2.2
                                 * self.blockSize, self.yPos + 8
                                 * self.blockSize))
                gameOverText1 = fontGAMEOVER.render('OVER', False,
                                                    BLACK)
                gameDisplay.blit(gameOverText1, (self.xPos + +2.35
                                 * self.blockSize, self.yPos + 12
                                 * self.blockSize))
    ...
    def draw_SCOREBOARD_BORDER(self):
        pygame.draw.rect(gameDisplay, BORDER_COLOR, [self.xPos
                         ...
                         + self.blockLineWidth, self.yPos
                         - self.boardLineWidth - self.blockLineWidth,
                         self.scoreBoardWidth + self.boardLineWidth,
                         self.boardLineWidth], 0)
        pygame.draw.rect(gameDisplay, BORDER_COLOR, [self.xPos
                         + self.blockSize * self.colNum
                         + self.boardLineWidth + self.blockLineWidth
                         + self.scoreBoardWidth, self.yPos
                         - self.boardLineWidth - self.blockLineWidth,
                         self.boardLineWidth, self.blockSize
                         * self.rowNum + 2 * self.boardLineWidth + 2
                         * self.blockLineWidth], 0)
        pygame.draw.rect(gameDisplay, BORDER_COLOR, [self.xPos
                         + self.blockSize * self.colNum
                         + self.blockLineWidth, self.yPos
                         + self.blockSize * self.rowNum
                         + self.blockLineWidth, self.scoreBoardWidth
                         + self.boardLineWidth, self.boardLineWidth], 0)
    ...
    def draw_SCOREBOARD_CONTENT(self):
        ...
        if self.gameStatus == 'running':
            ...
            gameDisplay.blit(nextPieceText, (xPosRef + self.blockSize,
                             self.yPos))
            ...
            for i in range(0, 4):
                ...
                if self.nextPieces[1] == 'O':
                    self.draw_BLOCK(xPosRef + 0.5 * self.blockSize,
                                    yPosRef + 2.25 * self.blockSize,
                                    blocks[i][ROW], blocks[i][COL],
                                    blockColors[self.nextPieces[1]])
                    ...
                    self.draw_BLOCK(xPosRef + 1 * self.blockSize,
                                    yPosRef + 2.25 * self.blockSize,
                                    blocks[i][ROW], blocks[i][COL],
                                    blockColors[self.nextPieces[1]])
            ...
            if self.gamePause == False:
                ...
                gameDisplay.blit(pauseText, (xPosRef + 1
                                 * self.blockSize, yLastBlock - 15
                                 * self.blockSize))
            else:
                unpauseText = fontSmall.render('P -> unpause', False,
                                               self.whiteSineAnimation())
                gameDisplay.blit(unpauseText, (xPosRef + 1
                                 * self.blockSize, yLastBlock - 15
                                 * self.blockSize))

            restartText = fontSmall.render('R -> restart', False, WHITE)
            gameDisplay.blit(restartText, (xPosRef + 1
                             * self.blockSize, yLastBlock - 14
                             * self.blockSize))
        else:

            yBlockRef = 0.3
            text0 = fontSB.render('press', False,
                                  self.whiteSineAnimation())
            gameDisplay.blit(text0, (xPosRef + self.blockSize,
                             self.yPos + yBlockRef * self.blockSize))
            text1 = fontSB.render('enter', False,
                                  self.whiteSineAnimation())
            gameDisplay.blit(text1, (xPosRef + self.blockSize,
                             self.yPos + (yBlockRef + 1.5)
                             * self.blockSize))
            text2 = fontSB.render('to', False,
                                  self.whiteSineAnimation())
            gameDisplay.blit(text2, (xPosRef + self.blockSize,
                             self.yPos + (yBlockRef + 3)
                             * self.blockSize))
            if self.gameStatus == 'firstStart':
                text3 = fontSB.render('start', False,
                                      self.whiteSineAnimation())
                gameDisplay.blit(text3, (xPosRef + self.blockSize,
                                 self.yPos + (yBlockRef + 4.5)
                                 * self.blockSize))
            else:
                text3 = fontSB.render('restart', False,
                                      self.whiteSineAnimation())
                gameDisplay.blit(text3, (xPosRef + self.blockSize,
                                 self.yPos + (yBlockRef + 4.5)
                                 * self.blockSize))

        pygame.draw.rect(gameDisplay, BORDER_COLOR, [xPosRef,
                         yLastBlock - 12.5 * self.blockSize,
                         self.scoreBoardWidth, self.boardLineWidth], 0)
    ...
    def whiteSineAnimation(self):
        ...
        sine = math.floor(255 * math.fabs(math.sin(2 * math.pi
                          * (gameClock.frameTick / (SINE_ANI_PERIOD
                                                    * 2)))))
    ...
    def lineClearAnimation(self):
        ...
        clearAniStage = math.floor((gameClock.frameTick
                                   - gameClock.clearAniStart)
                                   / CLEAR_ANI_PERIOD)
        ...
        if clearAniStage < halfCol:
            for i in range(0, 4):
                if self.clearedLines[i] >= 0:
                    ...
                    self.blockMat[self.clearedLines[i]][halfCol
                                                        + clearAniStage] = 'empty'

                    self.blockMat[self.clearedLines[i]][halfCol - 1
                                                        - clearAniStage] = 'empty'
        else:
    ...
    def dropFreeBlocks(self):  # Drops down the floating blocks after line clears occur
        ...
        for cLIndex in range(0, 4):
            if self.clearedLines[cLIndex] >= 0:
                for rowIndex in range(self.clearedLines[cLIndex], 0,
                                      -1):
                    for colIndex in range(0, self.colNum):

                        self.blockMat[rowIndex + cLIndex][colIndex] = \
                            self.blockMat[rowIndex + cLIndex
                                          - 1][colIndex]
    ...
    def checkAndApplyGameOver(self):
        if self.piece.gameOverCondition == True:
            ...
            for i in range(0, 4):
                if self.piece.blocks[i].currentPos.row >= 0 \
                        and self.piece.blocks[i].currentPos.col >= 0:
                    self.blockMat[self.piece.blocks[i].currentPos.row][self.piece.blocks[i].currentPos.col] = \
                        self.piece.type
...
class MovingPiece:
    ...
    def applyFastMove(self):
        ...
        if gameClock.move.check(gameClock.frameTick) == True:
            if self.lastMoveType == 'downRight' or self.lastMoveType \
                    == 'downLeft' or self.lastMoveType == 'down':
    ...
    def movCollisionCheck_BLOCK(self, dirType, blockIndex):
        if dirType == 'down':
            if self.blocks[blockIndex].currentPos.row + 1 > self.rowNum \
                - 1 \
                or self.blockMat[self.blocks[blockIndex].currentPos.row
                                 + directions[dirType][ROW]][self.blocks[blockIndex].currentPos.col
                                                             + directions[dirType][COL]] != 'empty':
                return True
        else:
            if directions[dirType][COL] \
                * (self.blocks[blockIndex].currentPos.col
                   + directions[dirType][COL]) > (self.colNum - 1
                                                  + directions[dirType][COL] * (self.colNum - 1)) / 2 \
                or self.blockMat[self.blocks[blockIndex].currentPos.row
                                 + directions[dirType][ROW]][self.blocks[blockIndex].currentPos.col
                                                             + directions[dirType][COL]] != 'empty':
                return True
    ...
    def rotCollisionCheck_BLOCK(self, blockCoor):
        if blockCoor[ROW] > self.rowNum - 1 or blockCoor[ROW] < 0 \
                or blockCoor[COL] > self.colNum - 1 or blockCoor[COL] < 0 \
                or self.blockMat[blockCoor[ROW]][blockCoor[COL]] != 'empty':
            return True
        return False
    ...
    def findOrigin(self):
        ...
        origin = [0, 0]
        origin[ROW] = self.blocks[0].currentPos.row \
            - self.currentDef[0][ROW]
        origin[COL] = self.blocks[0].currentPos.col \
            - self.currentDef[0][COL]
        return origin
        ...
        if self.type != 'O':
            ...
            for i in range(0, 4):
                if rotationType == 'CW':
                    tempBlocks[i][ROW] = origin[ROW] \
                        + self.currentDef[i][COL]
                    tempBlocks[i][COL] = origin[COL] + pieceMatSize - 1 \
                        - self.currentDef[i][ROW]
                else:
                    tempBlocks[i][COL] = origin[COL] \
                        + self.currentDef[i][ROW]
                    tempBlocks[i][ROW] = origin[ROW] + pieceMatSize - 1 \
                        - self.currentDef[i][COL]
            ...
            if self.rotCollisionCheck(tempBlocks) == False:
                for i in range(0, 4):
                    ...
                    self.currentDef[i][ROW] = \
                        self.blocks[i].currentPos.row - origin[ROW]
                    self.currentDef[i][COL] = \
                        self.blocks[i].currentPos.col - origin[COL]
...
def gameLoop():
    ...
    while not gameExit:  # Stay in this loop unless the game is quit
        if mainBoard.piece == "uncreated":
            ...
            for event in list(pygame.event.get()) + run_ai(mainBoard, mainBoard.piece, boardColNum, boardRowNum):
